...
class BookmarkDB {
  ...
  async deleteBookmark(id) {
    console.log("Attempting to delete bookmark with id:", id); // Add this line for debugging
    return new Promise((resolve, reject) => {
      const tx = this.db.transaction("bookmarks", "readwrite");
      const store = tx.objectStore("bookmarks");
      ...
      request.onsuccess = () => {
        console.log("Bookmark deleted successfully!"); // Add this line for debugging
        resolve();
      };
      request.onerror = () => {
        console.error("Failed to delete bookmark:", request.error); // Add this line for debugging
